...
static void _sim_state_view_update()
{
	...
	switch (s_info.handle_list.count) {
		case 1:
			sim1 = _sim_state_text_for_sim_get(s_info.handle_list.handle[0]);
			_sim_state_view_sliding_label_create(swipe_layout, sim1);
			break;
		case 2:
			sim1 = _sim_state_text_for_sim_get(s_info.handle_list.handle[0]);
			sim2 = _sim_state_text_for_sim_get(s_info.handle_list.handle[1]);
...
static void _sim_init(void)
{
	...
	int i, ret = telephony_init(&s_info.handle_list);
	if (ret == TELEPHONY_ERROR_NONE) {
		_E("Unable to initialize telephony");
		return;
	}

	if (s_info.handle_list.count <= 0) {
		_D("No available sim cards");
		return;
	}

	/* Register for Telephony call state changes */
	for (i = 0; i < s_info.handle_list.count; i++) {
		_sim_callbacks_register(s_info.handle_list.handle[i]);
	}
}

static void _sim_deinit(void)
{
	int ret = telephony_deinit(&s_info.handle_list);
	if (ret != TELEPHONY_ERROR_NONE) {
		_E("Unable to deinit telephony list");
	}
	s_info.handle_list.count = 0;
}
