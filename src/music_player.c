...
static void _music_player_minicontroler_event(minicontrol_event_e event, const char *minicontrol_name, bundle *event_arg, void *data)
{
    ...
    _D("Minicontrol name: %s", minicontrol_name);

	if (!strncmp(minicontrol_name, MUSIC_PLAYER_NAME, strlen(MUSIC_PLAYER_NAME))) {
		music_state = MUSIC_STATE_MUSIC_PLAYER_ON;
	} else if (!strncmp(minicontrol_name, SOUND_PLAYER_NAME, strlen(SOUND_PLAYER_NAME))) {
		music_state = MUSIC_STATE_SOUND_PLAYER_ON;
	} else {
		_D("minicontroller name : %s", minicontrol_name);
		return;
	}
}
...
int lock_music_player_init(void)
{
	...
	if (!_init_count++) {
		ret = minicontrol_viewer_set_event_cb(_music_player_minicontroler_event, NULL);
