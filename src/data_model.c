...
{
	LOCKSCREEN_DATA_MODEL_EVENT_BATTERY_CHANGED = ecore_event_type_new();
	...
	LOCKSCREEN_DATA_MODEL_EVENT_LCD_STATUS_CHANGED = ecore_event_type_new();
	...
	_E("Install all event handlers: %d", LOCKSCREEN_DATA_MODEL_EVENT_BATTERY_CHANGED);
}
...
int lockscreen_data_model_init()
{
	...
	int ret = lockscreen_data_model_battery_init(&model);
	if (ret) return -1;
	ret = lockscreen_data_model_background_init(&model);
...
{
	return ecore_event_add(event, &model, NULL, NULL) ? 0 : -1;
}
