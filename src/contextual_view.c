...
static Evas_Object *_contextual_view_scroller_create(Evas_Object *parent)
{
	Evas_Object *scroller = NULL;
...
static Evas_Object *_contextual_view_box_create(Evas_Object *parent)
{
...
#endif
static Evas_Object *_contextual_view_layout_create(Evas_Object *parent)
{
	...
	scroller = _contextual_view_scroller_create(layout);
	if (!scroller) {
...
static void _lock_contextual_view_destroy(void *data, Evas_Object *eo, Evas *e, void *event_info)
{
	struct contextual_data *data = data;
	lock_music_player_shutdown();
	free(data);
}

Evas_Object *lock_contextual_view_create(Evas_Object *parent)
{
	struct contextual_data *data = calloc(1, sizeof(contextual_view_t));
	if (!data) return NULL;

	lock_music_player_init();

	data->layout = _contextual_view_layout_create(parent);
	if (!data->layout) {
		free(data);
		return NULL;
	}
	data->scroller = _contextual_view_scroller_create(data->layout);
	if (!data->layout) {
	...
	data->box = _contextual_view_box_create(data->scroller);
	if (!data->layout) {
		evas_object_del(data->layout);
		free(data);
		return NULL;
	...
	elm_scroller_page_show(data->scroller, 1, 0);

	elm_object_signal_callback_add(data->layout, "btn,close,clicked", "btn.close", _close_btn_clicked_cb, data);
	evas_object_event_callback_add(data->layout, EVAS_CALLBACK_DEL, _lock_contextual_view_destroy, data);

	evas_object_data_set(data->layout, "contextual-data", data);
	return data->layout;
}
#endif
...
static struct contextual_data view;

static lock_contextual_event_state_changed_cb callback;
static void *user_data;

static void _player_state_cb(music_player_view_type_e type, bool on)
{
	if (on) {
...
int lock_contextual_view_init()
{
	lock_music_player_init();
	lock_music_player_state_changed_cb(_player_state_cb, NULL);

	return 0;
}

int lock_contextual_view_state_changed_cb_set(lock_contextual_event_state_changed_cb cb, void *data)
{
	callback = cb;
	user_data = data;
}
...
Evas_Object *_contextual_view_page_create(Evas_Object *parent)
{
	Evas_Object *page = NULL;
	Evas_Object *bg_rect = NULL;
	int page_width = 0;
	...
	if (!elm_layout_file_set(page, util_get_res_file_path(LOCK_EDJE_FILE), "music-page")) {
		_E("Failed to set edje file for page");
		evas_object_del(page);
		return NULL;
	}
...
static void _lock_contextual_view_destroy(void *data, Evas_Object *eo, Evas *e, void *event_info)
{
	struct contextual_data *cd = data;
	free(cd);
}

static void _update(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
	int x, y, w, h;
	_E("Geometry update");
...
static void _music_player_view_update(void)
{
	Evas_Object *controller = lock_music_player_view_create(MUSIC_PLAYER_VIEW_TYPE_SOUND, view.music_page);
	if (!controller)
	{
		_E("Fallback to music controller");
		controller = lock_music_player_view_create(MUSIC_PLAYER_VIEW_TYPE_MUSIC, view.music_page);
	}

	if (!controller)
		_E("Failed to create minicontrollers");

	elm_object_part_content_set(view.music_page, "sw.content", controller);

	evas_object_size_hint_weight_set(controller, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
	evas_object_size_hint_align_set(controller, EVAS_HINT_FILL, EVAS_HINT_FILL);

	elm_box_pack_start(view.box, view.music_page);
	evas_object_show(view.music_page);
	evas_object_show(controller);
}

Evas_Object *lock_contextual_view_layout_create(Evas_Object *parent)
{
	view.layout = _contextual_view_layout_create(parent);
	view.scroller = _contextual_view_scroller_create(view.layout);
	view.box = _contextual_view_box_create(view.scroller);
	view.music_page = _contextual_view_page_create(view.box);

	elm_object_part_content_set(view.layout, "sw.scroller", view.scroller);
	elm_object_content_set(view.scroller, view.box);
	elm_scroller_page_show(view.scroller, 1, 0);

	elm_object_signal_callback_add(view.layout, "btn,close,clicked", "btn.close", _close_btn_clicked_cb, &view);
	evas_object_event_callback_add(view.layout, EVAS_CALLBACK_DEL, _lock_contextual_view_destroy, &view);

	evas_object_data_set(view.layout, "contextual-view", &view);

	_music_player_view_update();

	return view.layout;
}


void lock_contextual_view_layout_destroy(void)
{
	evas_object_del(view.layout);
}

int lock_contextual_view_shutdown()
{
	lock_music_player_shutdown();
}
